...
class ProjectVariation(Base):
    ...
    variation_type = Column(
        Enum(VariationType, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True
    )
    status = Column(
        Enum(VariationStatus, values_callable=lambda x: [e.value for e in x]),
        nullable=False,
