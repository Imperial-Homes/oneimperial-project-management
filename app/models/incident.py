...
class ProjectIncident(Base):
    ...
    incident_type = Column(
        Enum(IncidentType, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True
    )
    severity = Column(
        Enum(IncidentSeverity, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True
    )
    status = Column(
