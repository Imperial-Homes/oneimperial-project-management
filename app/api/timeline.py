...
from app.models.timeline import ProjectTimeline, TimelineMilestone, TimelineTaskDependency
...
def create_timeline(
    timeline_data: ProjectTimelineCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    timeline = ProjectTimeline(**timeline_data.model_dump(), created_by=UUID(current_user["sub"]))
    db.add(timeline)
    db.commit()
    db.refresh(timeline)
    return timeline
...
def get_timeline(timeline_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get timeline by ID."""
    timeline = (
        db.query(ProjectTimeline)
        .options(joinedload(ProjectTimeline.dependencies))
        .filter(ProjectTimeline.id == timeline_id)
        .first()
    )
...
def delete_timeline(timeline_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Delete timeline."""
    timeline = db.query(ProjectTimeline).filter(ProjectTimeline.id == timeline_id).first()
    if not timeline:
        raise HTTPException(status_code=404, detail="Timeline not found")
...
def create_task_dependency(
    dependency_data: TaskDependencyCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
...
def get_task_dependency(
    dependency_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
...
def create_milestone(
    milestone_data: MilestoneCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    milestone = TimelineMilestone(**milestone_data.model_dump(), created_by=UUID(current_user["sub"]))
    db.add(milestone)
    db.commit()
...
def get_milestone(milestone_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get milestone by ID."""
...
def delete_milestone(milestone_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Delete milestone."""
    milestone = db.query(TimelineMilestone).filter(TimelineMilestone.id == milestone_id).first()
    if not milestone:
        raise HTTPException(status_code=404, detail="TimelineMilestone not found")
...
def get_gantt_chart_data(
    ...
    dependencies = (
        db.query(TimelineTaskDependency).filter(TimelineTaskDependency.predecessor_task_id.in_(task_ids)).all()
    )
    ...
    if tasks:
        min_date = min(
            (t.start_date for t in tasks if t.start_date), default=project.start_date or datetime.utcnow().date()
        )
        max_date = max(
            (t.due_date for t in tasks if t.due_date), default=project.target_end_date or datetime.utcnow().date()
        )
