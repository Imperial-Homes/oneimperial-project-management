...
def get_resource_utilization(
    utilization_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Get resource utilization by ID."""
    utilization = db.query(ResourceUtilization).filter(ResourceUtilization.id == utilization_id).first()
...
def get_resource_utilization_history(
    ...
    query = db.query(ResourceUtilization).filter(ResourceUtilization.resource_id == resource_id)
...
def get_current_resource_utilization(
    resource_id: UUID, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Get current period resource utilization."""
    now = datetime.utcnow()

    utilization = (
        db.query(ResourceUtilization)
        .filter(
    ...
    if not utilization:
        raise HTTPException(status_code=404, detail="No utilization record found for current period")
...
def get_resource_utilization_summary(
    ...
    current_utilization = (
        db.query(ResourceUtilization)
        .filter(ResourceUtilization.resource_id == resource_id)
        .order_by(desc(ResourceUtilization.period_start))
        .first()
    )
...
def get_all_resources_utilization(
    ...
    min_utilization: float | None = Query(None, ge=0, le=100),
    max_utilization: float | None = Query(None, ge=0, le=100),
    period_days: int = Query(30, ge=1, le=365),
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    for resource in resources:
        ...
        current_utilization = (
            db.query(ResourceUtilization)
            .filter(ResourceUtilization.resource_id == resource.id)
            .order_by(desc(ResourceUtilization.period_start))
            .first()
        )
