...
def get_latest_project_progress(
    ...
    ).order_by(desc(ProjectProgress.recorded_date)).first()
...
def get_task_progress_history(
    ...
    progress_history = db.query(TaskProgress).filter(
        TaskProgress.task_id == task_id
    ).order_by(desc(TaskProgress.recorded_date)).limit(limit).all()
